...
@Injectable()
export class FileUploaderService extends BaseAppService {

  public url: string = '/Uploads/Review/Photo';
  ...
  constructor(protected http: HttpClient) {
		super(http);
		this.serviceUri = `${environment.coreServiceApiUrlV2}`;
  ...
  private _addToQueue(file: any) {
    ...
    reader.onload = (e: any) => {
      ...
      queueObj.file.imgUrl = e.target.result;
    }
  ...
  private _removeFromQueue(queueObj: FileQueueObject) {
    ...
    this._files.forEach(element => {
      this.onCompleteItem(element, null);
    });
  }
  ...
  private _upload(queueObj: FileQueueObject) {
    ...
    const req = new HttpRequest('POST', this.serviceUri+this.url, form, {
      headers: this.getMultiPartAuthHttpRequestHeader(),
      reportProgress: true,
    });
